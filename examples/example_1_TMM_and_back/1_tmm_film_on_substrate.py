...
from TRANK import TRA_spectra, try_mkdir
...
spectra = array( TRA_spectra(lamda_list = lamda_list,
					nk_f = nk_f_list[layer_index_of_fit],  # we may have buit our parameter_list_generator to have our layer of interest in it, but these functions are all built around analyze variations of our layer nk
					parameter_list_generator = parameter_list_generator) )

#####spectra[lamda index][ param number][ T/R/A spectrum index] for reference!
...
for angle_index in range(len(angle_list)):
	...
	T_list = spectra[:,angle_index,0] + noise* (rand(len(lamda_list))-0.5)
	R_list = spectra[:,angle_index,1] + noise* (rand(len(lamda_list))-0.5)
	A_list = spectra[:,angle_index,2] + noise* (rand(len(lamda_list))-0.5)
